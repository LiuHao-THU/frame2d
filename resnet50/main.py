...
with tf.Session() as sess:
    with tf.variable_scope("model") as scope:
        ...
        model = ResNet(vgg16_npy_path = configs['model_path'])
        model.build(rgb = images, train=True, num_classes=3, random_init_fc8=False, debug=False, use_dilated=False)
        # z1 = model.build(images, configs['num_classes'], train_mode, configs['final_layer_type'])
        # scope.reuse_variables()
        # z2 = model.build(eval_images, LABELSNUM, train_mode, configs['final_layer_type'])
        ...
        for epoch in range(configs['epoch']):
            ...
            for batch in read.minibatches_train(batch_size = configs['batch_size'], shuffle = True):
                ...
                temp_cost, _ ,c= sess.run([cost, train, model.y_soft], feed_dict={images: images_batch, labels: labels_batch, train_mode: True})
